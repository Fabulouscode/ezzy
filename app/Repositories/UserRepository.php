...
class UserRepository extends Repository
{
    ...
    public function getPatientDetailWithRelationship($request)
    {
        ...
        if (!empty($request->appointment_start_date) && !empty($request->appointment_end_date)) {
            $appendQuery .= ' AND appointment.created_at BETWEEN ' . $request->appointment_start_date . ' AND ' . $request->appointment_end_date;
        }

        if (isset($request->appointment_type) && $request->appointment_type != '') {
            $appendQuery .= ' AND appointment.appointment_type = ' . $request->appointment_type;
        }

        if (isset($request->urgent) && $request->urgent != '') {
            $appendQuery .= ' AND appointment.urgent = ' . $request->urgent;
        }

        $categoryCondition = '';
        if (!empty($request->category_id)) {
            $categoryCondition = ' AND hcp.category_id = ' . $request->category_id;
        }

        // Construct the query
        $query = User::query()
            ->select('users.*', DB::raw("
                (SELECT COUNT(*) FROM appointments as appointment
                    LEFT JOIN users AS hcp ON appointment.user_id = hcp.id 
                    WHERE appointment.client_id = users.id AND appointment.status = 5 $appendQuery $categoryCondition
                ) AS client_appointments_count,
                (SELECT COUNT(*) FROM orders 
                    WHERE client_id = users.id AND status = 3
